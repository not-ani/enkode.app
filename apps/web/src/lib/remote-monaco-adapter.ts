...
export function registerRemoteMonacoAdapter<Language extends RemoteLanguage>(
  monaco: typeof Monaco,
  service: RemoteLanguageServiceContract<Language>,
  workspaceId: string,
  language: Language,
) {
  const markerOwner = `enkode-${language}`;
  ...
  const diagnosticSubscription = service.subscribeDiagnostics((diagnostics) => {
    const grouped = new Map<string, LanguageDiagnostic[]>();
...
function completionKind(monaco: typeof Monaco, completion: LanguageCompletion) {
  return {
